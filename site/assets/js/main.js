...
function mountSlider(track, arrows, itemSel) {
  ...
  // размер ленты меняется и без смены размера окна: догружаются шрифты и стили. Без наблюдения
  // стрелки оставались в состоянии первого замера, например погашенными у длинной ленты команды
  if ("ResizeObserver" in window) {
    const watch = new ResizeObserver(update);
    [track, ...track.querySelectorAll(itemSel)].forEach((el) => watch.observe(el));
  } else {
...
function initHome() {
  ...
  carousel.innerHTML = DIRECTIONS.map((raw) => [raw, tr("directions", raw.id, raw)]).map(([raw, d]) => `
    <a class="dir-card" href="direction.html?id=${d.id}">
      ...
      <div class="dir-card__img${d.image ? "" : " is-empty"}">${d.image ? `<img src="${img(d.image)}" alt="" loading="lazy" data-pos="${esc(d.imagePos || "center")}">` : ""}</div>
    </a>`).join("");

  // кадрирование задаем через свойство, а не атрибутом style: атрибуты стилей запрещает CSP
  carousel.querySelectorAll("img[data-pos]").forEach((i) => { i.style.objectPosition = i.dataset.pos; });
...
/* Метрики направления считаются по его же публикациям, а не берутся из анкет:
   цитирования и квартили приходят из базы работ (tools/publications/enrich.py). */
...
function initDirection() {
  ...
  if (d.people) {
    // фотографии сотрудников убраны (решение владельца 21.09.2026): карточка это плашка фирменного цвета
    people.innerHTML = teamOf(d).map((p) => `
      <figure class="person person--plain">
