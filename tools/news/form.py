"""Переносит новости из анкеты «011_Новости.docx» (папка data/news) в content/news.json.
...
Разобранные новости лежат в content/news.json, это источник правды: анкету владелец
...
Запуск: python tools/news/form.py [--dry] [--form ПУТЬ]
"""
import argparse
import datetime
import json
...
from lib.paths import FORM, MANUAL_NEWS_TITLES as TITLES, NEWS as STORE
...
# Заголовки, переписанные для сайта (content/manual/news_titles.json) ведет tools/news/titles.py,
# здесь они побеждают анкету: в анкете заголовок пишут как удобно, а на карточке он должен
# читаться как в газете
...
def main():
    ...
    for number, mark, fields in read_form(args.form):
        ...
        apply(record, fields)
        # номер таблицы в анкете: по нему tools/news/photos.py раскладывает фотографии
        record["form"] = number
        if number in DATES:
    ...
    if args.dry:
        ...
        return
    write_store(items)
    print("записано:", rel(STORE))
    print("дальше: python tools/news/photos.py (если есть новые снимки) и python tools/smoke.py")
