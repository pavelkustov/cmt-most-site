"""Собирает авторские абстракты публикаций в content/abstracts.json.
...
Источники: content/collected/publications_db.json (там абстракты из OpenAlex) и Crossref по DOI
для тех работ, у которых в базе абстракта нет. Crossref отдает его разметкой JATS,
теги вычищаются, абзацы сохраняются.

Файл получается вида {"10.1021/...": "текст"}, сборка кладет его в assets/js/abstracts.js,
он подключается на страницах со списками публикаций.

Запуск: python tools/publications/abstracts.py [--offline] [--limit N] [--import-md]
"""
import argparse
import html
import pathlib
...
from lib.paths import ABSTRACTS as OUT, EDIT, MANUAL_ABSTRACTS as MANUAL, PUBLICATIONS, PUBLICATIONS_DB as DB
from lib.store import load, rel, save, write
from lib.web import get_json

MISSING = EDIT / "ABSTRACTS_MISSING.md"   # список работ, которым абстракта не нашлось
NL = chr(10)
CACHE = "cmt_most_abstracts"
...
def site_dois():
    ...
    return {p["doi"].removeprefix("https://doi.org/").lower() for p in load(PUBLICATIONS) if p.get("doi")}


def fetch(url, tries=2, pause=0.0):
    """Запрос с кэшем на диске: повторный сбор не дергает чужие сервисы заново."""
    return get_json(url, cache=CACHE, tries=tries, pause=pause, offline=bool(NO_NET))
...
def load_manual(raw=False):
    ...
    rows = {k.lower(): v for k, v in load(MANUAL, {}).items() if v.strip()}
    return rows if raw else {k: polish(v) for k, v in rows.items()}


def import_md(db):
    """Забирает тексты, вписанные в content/edit/ABSTRACTS_MISSING.md, в content/manual/abstracts.json."""
...
def main():
    ...
    ap.add_argument("--import-md", dest="import_md", action="store_true",
                    help="забрать тексты из content/edit/ABSTRACTS_MISSING.md и пересобрать")
    args = ap.parse_args()
    ...
    save(OUT, found, sort_keys=True)
    left = len(need) - len(found)
    print(f"записано: {rel(OUT)} ({len(found)} абстрактов, {OUT.stat().st_size // 1024} КБ), без абстракта: {left}")
