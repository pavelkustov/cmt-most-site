"""Собирает публикации сотрудников центра из OpenAlex в content/collected/publications_db.json.

Идентификаторы (ORCID, Scopus ID, Researcher ID, Google Scholar) берутся из книги
«ЦМТ Мост.xlsx» (папка data/book) и в репозиторий не попадают: в базу идут только
...
Запуск: python tools/publications/collect.py [--xlsx путь] [--report путь]
"""
import argparse
import difflib
import hashlib
...
from lib.paths import BOOK as XLSX, DIRECTIONS, PUBLICATIONS, PUBLICATIONS_DB as OUT
...
def short_ru(full):
    """«Зюзин Михаил Валерьевич» -> «Михаил Зюзин» (так люди названы в данных сайта)."""
    parts = full.split()
...
def site_data():
    ...
    dirs = {}
    for d in load(DIRECTIONS):
        for name in d.get("people", []):
            dirs.setdefault(name, []).append(d["id"])
    dois = {p["doi"].removeprefix("https://doi.org/").lower() for p in load(PUBLICATIONS) if p.get("doi")}
    return dirs, dois
...
EXTRA = {
    ...
    "10.1109/iclo69056.2026.11624504": ["Николай Жесткий", "Артем Ларин"],   # у Ларина тоже в профиле
