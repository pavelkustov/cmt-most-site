"""Собирает английские страницы en/*.html из русских.
...
кириллица (поменяли русский текст, а пару не обновили), скрипт падает и показывает где.

Запуск перед bump_assets.py: python tools/build_en.py
"""
import pathlib
import re
import sys
...
OUT = ROOT / "en"
SITE = "https://pavelkustov.github.io/cmt-most-site/"
...
def with_hreflang(text, name):
    """Ссылки на обе версии сразу после canonical, старые убираем (скрипт запускают много раз)"""
    text = HREFLANG.sub("", text)
    links = "".join(f'\n  <link rel="alternate" hreflang="{lang}" href="{page_url(name, code)}">'
...
def build(name):
    src = ROOT / name
    ru = src.read_text(encoding="utf-8")
    ru_new = with_hreflang(ru, name)
    if ru_new != ru:
        src.write_text(ru_new, encoding="utf-8", newline="\n")
    text = re.sub(r"\s*<!--.*?-->", "", ru_new, flags=re.S)   # комментарии для разработчика, в них русский
    ...
    text = re.sub(r'(<link rel="canonical" href=")[^"]*(")', lambda m: m.group(1) + page_url(name, "en") + m.group(2), text)
    text = re.sub(r'(<meta property="og:url" content=")[^"]*(")', lambda m: m.group(1) + page_url(name, "en") + m.group(2), text)
    ...
    text = text.replace("<!doctype html>", f"<!doctype html>\n<!-- Generated by tools/build_en.py from ../{name}. Do not edit by hand. -->", 1)
...
failed = False
for name in PAGES:
    unused, left = build(name)
    print(f"en/{name}: готово" + (f", не пригодились пары: {len(unused)}" if unused else ""))
    for u in unused:
        # общие пары есть не на каждой странице, это нормально
        if not any(u == c[0] for c in COMMON):
            print("   ~ пара не нашлась в русской странице:", u[:90])
    if left:
        failed = True
        print("   ! осталась кириллица, допишите пары в tools/build_en.py:")
        for line in left:
            print("     ", line)
