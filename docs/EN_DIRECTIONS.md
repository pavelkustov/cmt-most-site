...
`python tools/en_content.py --import` (собирает `assets/js/en-content.js`), затем
`python tools/build_en.py` и `python tools/bump_assets.py`.

Перевод сделан 24.09.2026 по русским текстам из `docs/DIRECTIONS_TEXT.md`, вычитывает владелец.
...
по-английски лежат в `docs/EN_PEOPLE.md`.
